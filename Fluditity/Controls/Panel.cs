...
namespace Fluid.Controls
{
    ...
    public class FluidPanel : ControlContainer, ILayoutPanel
    {
        ...
        protected override void OnPaintBackground(FluidPaintEventArgs e)
        {
            ...
            //if (IsOpaque || HasCustomPaintBackgound) base.OnPaintBackground(e);
            //else
            //{
            //    CreateTransparentBGColor();
            //    using (GraphicsPlus gp = new GraphicsPlus(e.Graphics))
            //    {
            //        using (SolidBrushPlus brush = new SolidBrushPlus(transparentBgColor))
            //        {
        ...
        private void StopToLeftAnimation()
        {
            if (toleftAnimation != null) toleftAnimation.Stop();
        }
        ...
        public void AnimateToLeft(int duration, bool wait)
        {
            StopToLeftAnimation();
            if (bounds.X != 0)
            {
                Visible = true;
                Animation animation = EnsureToLeftAnimation();
                animation.Duration = duration;
                animation.BeginValue = bounds.X;
                animation.EndValue = 0;

                if (wait) animation.StartModal(); else animation.Start();
            }
        }
        public void AnimateToLeftOff(int duration, bool wait)
        {
            StopToLeftAnimation();
            if (bounds.X != 0)
            {
                ...
                animation.EndValue = -bounds.Width;

                if (wait) animation.StartModal(); else animation.Start();
            }
        }
        public void AnimateToRight(int duration, bool wait)
        {
            StopToLeftAnimation();
            if (bounds.X != 0)
            {
                ...
                Animation animation = EnsureToLeftAnimation();
                animation.Duration = duration;
                animation.BeginValue = -bounds.Width;
                animation.EndValue = 0;
        ...
        private Animation EnsureTopAnimation()
        {
            if (topAnimation == null)
            {
                ...
                topAnimation.Completed += new EventHandler(animation_Completed);
                topAnimation.Scene += new EventHandler<AnimationEventArgs>(animation_Scene);
                topAnimation.Started += new EventHandler(topAnimation_Started);
            }
            return topAnimation;
        }

        private Animation EnsureToLeftAnimation()
        {
            if (toleftAnimation == null)
            {
                toleftAnimation = new Animation();
                toleftAnimation.Mode = AnimationMode.Log;
                toleftAnimation.Acceleration = 0.05f;
                toleftAnimation.Completed += new EventHandler(animation_Completed);
                toleftAnimation.Scene += new EventHandler<AnimationEventArgs>(ToLeftAnimation_Scene);
                toleftAnimation.Started += new EventHandler(toLeftAnimation_Started);
            }

            return toleftAnimation;
        }
        void toLeftAnimation_Started(object sender, EventArgs e)
        {
            Update();
        }
        void topAnimation_Started(object sender, EventArgs e)
        {
            Update();
        }
        ...
        private Animation EnsureAlphaAnimation()
        {
            if (alphaAnimation == null)
            {
                ...
                alphaAnimation.Completed += new EventHandler(animation_Completed);
                alphaAnimation.Scene += new EventHandler<AnimationEventArgs>(alphaAnimation_Scene);
                alphaAnimation.Started += new EventHandler(topAnimation_Started);
            }
        ...
        void ToLeftAnimation_Scene(object sender, AnimationEventArgs e)
        {
            AnchorStyles anchor = Anchor;
            try
            {
                Anchor = AnchorStyles.Left | AnchorStyles.Top;
                bounds.X = e.Value;this.Refresh();
            }
            finally
            {
                Anchor = anchor;
        ...
        public virtual void Show(ShowTransition transition)
        {
            ...
            switch (transition)
            {
                ...
                case ShowTransition.FromRight:
                    AnimateToLeft(Animation.DefaultDuration, modal);
                    break;
        ...
        public virtual void Close(ShowTransition transition)
        {
            ...
            switch (transition)
            {
                ...
                case ShowTransition.FromBottom:
                    topAnimation.Completed += new EventHandler(topAnimation_Completed);
                ...
                case ShowTransition.FromTop:
                    ...
                    break;
                case ShowTransition.ToLeft:
                    topAnimation.Completed += new EventHandler(topAnimation_Completed);
                    AnimateToLeftOff(Animation.DefaultDuration, modal);
                    break;
                default:
                    throw new NotImplementedException();
            }
